...
bucket_name = 'imagedumpv2'
queue_url = 'https://sqs.us-east-1.amazonaws.com/825767410344/Queue1'
        ...
        if messages:
            ...
            for message in messages:
                ...
                if message['Body'].startswith("s3://"):
                    ...
                    bucket, object_name = s3_location.replace(
                        "s3://", "").split("/", 1)
                    new_file_name = os.path.join(os.path.expanduser(
                        "~"), "Desktop", "processed_" + os.path.basename(object_name))
                    ...
                    final_image_path = os.path.join(
                        ...
                        "C:\\Users\\karee",  f"final_image_{file_name}")
...
def select_processing_option(root):
    ...
    operations = ["blur", "Erosion", "Dilate",
                  "colorinversion", "edgedetection"]
    for op in operations:
        button = tk.Button(
            option_window, text=op, command=lambda op=op: choose_option(op))
        button.pack(pady=10)
    ...
    if file_path:
        ...
        if operation:  # Only proceed if an operation is selected
            ...
            message_body = json.dumps(
                {'s3_location': f"s3://{bucket_name}/{img_file}", 'operation': operation})
...
def main():
    ...
    # make button wider
    upload_button = tk.Button(root, text="Upload Image", command=lambda: upload_image(
